...
async function retrieveTtsVolume () {
  return await db.tts.get('tts_volume')
}

async function setTtsVolume () {
  const ttsVolume = await retrieveTtsVolume()
  rangeInput.value = ttsVolume
}

async function setTtsVolumeDb (volume = 80) {
...
async function setTtsVolumeVisability () {
  ...
  if (tts == undefined || tts === true) {
    ...
    if (!existingDetails) {
      ...
      rangeInput.setAttribute('id', 'tts_volume')
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  rangeInput.addEventListener('input', (event) => {
    ...
    setTtsVolumeDb(event.target.value)
  })
})
